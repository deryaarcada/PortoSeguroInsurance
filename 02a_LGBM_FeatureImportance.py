...
Compute LightGBM feature importances using 5-fold CV (matching notebook methodology).

Behavior:
- Loads preprocessed `X_preprocessed.csv` and `y_preprocessed.csv`.
- Runs 5-fold stratified cross-validation with eski notebook parameters.
- Applies target encoding to ps_car_11_cat (leakage-free per fold).
- Extracts feature importances from each fold and averages them.
- Saves `model_results/lgbm_feature_importance.csv` and visualization.
...
# Old notebook LGBM params (matching eski methodology)
LGBM_PARAMS = dict(
    n_estimators=1000,
    learning_rate=0.01,
    num_leaves=7,
...
def target_encode(train_series, target_series, val_series, smoothing=10):
    ...
    #Leakage-free target encoding (fit on train, apply to val/test).
    ...
    for category in train_series.unique():
        ...
        sum_target = target_series[mask].sum()
        count_target = mask.sum()
        
        # Apply smoothing (lambda regularization)
        smooth_coef = 1.0 / (1.0 + np.exp(-(count_target - smoothing)))
        encoding_dict[category] = (
            sum_target / count_target * smooth_coef + 
            global_mean * (1 - smooth_coef)
        )
    
    # Apply mapping to val_series
    val_encoded = val_series.map(encoding_dict).fillna(global_mean)
    return train_series.astype(float), val_encoded.astype(float)
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # ===== TARGET ENCODING (ps_car_11_cat only) =====
        if 'ps_car_11_cat' in X_train_f.columns:
        ...
        cat_feats = [c for c in categorical_cols if c in X_train_f.columns and c != 'ps_car_11_cat']
        for c in cat_feats:
            X_train_f[c] = X_train_f[c].astype('category')
            X_val_f[c] = X_val_f[c].astype('category')
