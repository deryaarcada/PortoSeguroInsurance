...
The pipeline is modularized into dedicated Python scripts, enforcing clean separation of concerns and reproducibility. *The pipeline is orchestrated end-to-end via `run_pipeline.py`, which runs 6 stages in sequence and logs timing/status for each.*
...
* **`01_Preprocessing.py`**
  ...
  * Drops the 20 noisy `ps_calc` columns and casts categorical variables to Pandas `category` dtype.
...
* **`02b_HyperParameterTuning.py`** *(previously `02c_HyperParameterTuning.py`)*
...
*Note: File and folder names above reflect the pipeline's current state, as run via `run_pipeline.py`.*
...
```bash
pip install pandas numpy lightgbm scikit-learn joblib tqdm shap optuna matplotlib seaborn xgboost
```
