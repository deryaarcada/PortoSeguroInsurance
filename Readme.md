...
* **Independent External Validation (Kaggle):** Achieved a **Public Gini Score of 0.27632** and a **Private Gini Score of 0.28394**. The upward shift in the hidden private set demonstrates strong generalization and absolute resilience against overfitting.
...
* **Actuarial-Grade Calibration:** Post-calibration Expected Calibration Error (ECE) was minimized to **0.00043** using Isotonic Regression, ensuring predicted probabilities mirror empirical risk frequencies.
...
### 🎛️ 3. Optimization & Rigorous Evaluation
...
* **`03c_Business_Impact_Sensitivity.py`**
  * Simulates a real-world underwriting framework mapping predicted probabilities against expected financial loss.
...
# 🚗 Porto Seguro's Safe Driver Prediction Pipeline

An end-to-end, production-grade machine learning pipeline developed for the Porto Seguro Safe Driver Prediction dataset. This project aims to predict the probability that a driver will initiate an auto insurance claim in the next year. It implements rigorous data preprocessing, feature engineering, hyperparameter optimization, probability calibration, SHAP interpretability, and business impact/sensitivity analysis.
...
*   **`01_Preprocessing.py`**
    ...
    *   **Outputs Written To:**
        *   `outputs/X_preprocessed.csv` (Features for training)
        *   `outputs/y_preprocessed.csv` (Target array)
        *   `outputs/test_preprocessed.csv` (Features for deployment/submission)
        *   `outputs/metadata.pkl`, `categorical_cols.pkl`, `rare_class_map.pkl` (Metadata & Maps)
